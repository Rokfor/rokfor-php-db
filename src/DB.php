...
$this->defaultLogger->info("PRIVATE: " . $private);
  ...
  function searchContributions($string, $issueid = false, $chapterid = false, $status = false, $limit = false, $offset = false, $filterfield = false, $filtermode = "like", $sortmode = 'asc', $count = false, $templateid = false) {
    ...
    if ($count === true) return $this->ContributionsQuery()
                ...
                ->_if($filterby != "")
                  ->_if($filtermode == "like")
                    ->$filterby(is_array($string) ? $string : '%'.$string.'%')
                  ->_endif()
                  ->_if($filtermode == "lt")
                    ->addUsingAlias($filterbycol, is_array($string) ? $string : (int)$string, \Propel\Runtime\ActiveQuery\Criteria::LESS_THAN)
                  ->_endif()
                  ->_if($filtermode == "gt")
                    ->addUsingAlias($filterbycol, is_array($string) ? $string : (int)$string, \Propel\Runtime\ActiveQuery\Criteria::GREATER_THAN)
                  ->_endif()
                  ->_if($filtermode == "lte")
                    ->$filterby(array('max' => is_array($string) ? $string : (int)$string))
                  ->_endif()
                  ->_if($filtermode == "gte")
                    ->$filterby(array('min' => is_array($string) ? $string : (int)$string))
                  ->_endif()
                  ->_if($filtermode == "eq")
                    ->$filterby(is_array($string) ? $string : (int)$string)
                  ->_endif()

                ->_else()
                  ->_if(!$filterfield)
                    ->filterByName(is_array($string) ? $string : '%'.$string.'%')
                    ->_or()
                  ->_endif()
                  ->useDataQuery()
                      ->_if($filterfield)
                      ...
                      ->_endif()
                      ->_if($filtermode == "like")
                        ->filterByContent(is_array($string) ? $string : '%'.$string.'%')
                      ->_endif()
                      ->_if($filtermode == "lt")
