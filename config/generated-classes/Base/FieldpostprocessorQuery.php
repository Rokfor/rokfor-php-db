...
abstract class FieldpostprocessorQuery extends ModelCriteria
{
    ...
    public static function purgeCache()
    {
        ...
        $driver->setNamespace(FieldpostprocessorTableMap::TABLE_NAME);
        ...
        if ($result !== null) {
            if ($result instanceof \ArrayAccess) {
                foreach ($result as $element) {
                    if ($element instanceof \Fieldpostprocessor) {
                        FieldpostprocessorTableMap::addInstanceToPool($element);
                    }
                }
            } else if ($result instanceof \Fieldpostprocessor) {
                FieldpostprocessorTableMap::addInstanceToPool($result);
            }
        }
    ...
    {
        if ($this->isCacheEnable() && $cache = \FieldpostprocessorQuery::cacheFetch($this->getCacheKey())) {
        ...
        if ($this->isCacheEnable()) {
            \FieldpostprocessorQuery::cacheStore($this->getCacheKey(), $data, $this->getLifeTime());
        }
    ...
    {
        if ($this->isCacheEnable() && $cache = \FieldpostprocessorQuery::cacheFetch($this->getCacheKey())) {
            if ($cache instanceof \Fieldpostprocessor) {
                return $cache;
            }
        }
