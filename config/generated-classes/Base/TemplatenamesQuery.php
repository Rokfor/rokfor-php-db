...
abstract class TemplatenamesQuery extends ModelCriteria
{
    ...
    public static function purgeCache()
    {
        ...
        $driver->setNamespace(TemplatenamesTableMap::TABLE_NAME);
        ...
        if ($result !== null) {
            if ($result instanceof \ArrayAccess) {
                foreach ($result as $element) {
                    if ($element instanceof \Templatenames) {
                        TemplatenamesTableMap::addInstanceToPool($element);
                    }
                }
            } else if ($result instanceof \Templatenames) {
                TemplatenamesTableMap::addInstanceToPool($result);
            }
        }
    ...
    {
        if ($this->isCacheEnable() && $cache = \TemplatenamesQuery::cacheFetch($this->getCacheKey())) {
        ...
        if ($this->isCacheEnable()) {
            \TemplatenamesQuery::cacheStore($this->getCacheKey(), $data, $this->getLifeTime());
        }
    ...
    {
        if ($this->isCacheEnable() && $cache = \TemplatenamesQuery::cacheFetch($this->getCacheKey())) {
            if ($cache instanceof \Templatenames) {
                return $cache;
            }
        }
