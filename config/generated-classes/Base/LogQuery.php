...
abstract class LogQuery extends ModelCriteria
{
    ...
    public static function purgeCache()
    {
        ...
        $driver->setNamespace(LogTableMap::TABLE_NAME);
            ...
            if ($result instanceof \ArrayAccess) {
                foreach ($result as $element) {
                    if ($element instanceof \Log) {
                        LogTableMap::addInstanceToPool($element);
                    }
                }
            } else if ($result instanceof \Log) {
                LogTableMap::addInstanceToPool($result);
            }
        }
    ...
    {
        if ($this->isCacheEnable() && $cache = \LogQuery::cacheFetch($this->getCacheKey())) {
        ...
        if ($this->isCacheEnable()) {
            \LogQuery::cacheStore($this->getCacheKey(), $data, $this->getLifeTime());
        }
