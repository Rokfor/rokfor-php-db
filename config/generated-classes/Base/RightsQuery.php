...
abstract class RightsQuery extends ModelCriteria
{
    ...
    public static function purgeCache()
    {
        ...
        $driver->setNamespace(RightsTableMap::TABLE_NAME);
            ...
            if ($result instanceof \ArrayAccess) {
                foreach ($result as $element) {
                    if ($element instanceof \Rights) {
                        RightsTableMap::addInstanceToPool($element);
                    }
                }
            } else if ($result instanceof \Rights) {
                RightsTableMap::addInstanceToPool($result);
            }
        }
    ...
    {
        if ($this->isCacheEnable() && $cache = \RightsQuery::cacheFetch($this->getCacheKey())) {
        ...
        if ($this->isCacheEnable()) {
            \RightsQuery::cacheStore($this->getCacheKey(), $data, $this->getLifeTime());
        }
