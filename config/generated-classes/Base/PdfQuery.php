...
abstract class PdfQuery extends ModelCriteria
{
    ...
    public static function purgeCache()
    {
        ...
        $driver->setNamespace(PdfTableMap::TABLE_NAME);
        ...
        if ($result !== null) {
            if ($result instanceof \ArrayAccess) {
                foreach ($result as $element) {
                    if ($element instanceof \Pdf) {
                        PdfTableMap::addInstanceToPool($element);
                    }
                }
            } else if ($result instanceof \Pdf) {
                PdfTableMap::addInstanceToPool($result);
            }
        }
    ...
    {
        if ($this->isCacheEnable() && $cache = \PdfQuery::cacheFetch($this->getCacheKey())) {
        ...
        if ($this->isCacheEnable()) {
            \PdfQuery::cacheStore($this->getCacheKey(), $data, $this->getLifeTime());
        }
