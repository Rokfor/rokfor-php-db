...
abstract class BatchQuery extends ModelCriteria
{
    ...
    public static function purgeCache()
    {
        ...
        $driver->setNamespace(BatchTableMap::TABLE_NAME);
            ...
            if ($result instanceof \ArrayAccess) {
                foreach ($result as $element) {
                    if ($element instanceof \Batch) {
                        BatchTableMap::addInstanceToPool($element);
                    }
                }
            } else if ($result instanceof \Batch) {
                BatchTableMap::addInstanceToPool($result);
            }
        }
    ...
    {
        if ($this->isCacheEnable() && $cache = \BatchQuery::cacheFetch($this->getCacheKey())) {
        ...
        if ($this->isCacheEnable()) {
            \BatchQuery::cacheStore($this->getCacheKey(), $data, $this->getLifeTime());
        }
