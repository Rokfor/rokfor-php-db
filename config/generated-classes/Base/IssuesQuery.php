...
abstract class IssuesQuery extends ModelCriteria
{
    ...
    public static function purgeCache()
    {
        ...
        $driver->setNamespace(IssuesTableMap::TABLE_NAME);
        ...
        if ($result !== null) {
            if ($result instanceof \ArrayAccess) {
                foreach ($result as $element) {
                    if ($element instanceof \Issues) {
                        IssuesTableMap::addInstanceToPool($element);
                    }
                }
            } else if ($result instanceof \Issues) {
                IssuesTableMap::addInstanceToPool($result);
            }
        }
    ...
    {
        if ($this->isCacheEnable() && $cache = \IssuesQuery::cacheFetch($this->getCacheKey())) {
        ...
        if ($this->isCacheEnable()) {
            \IssuesQuery::cacheStore($this->getCacheKey(), $data, $this->getLifeTime());
        }
