...
abstract class PluginsQuery extends ModelCriteria
{
    ...
    public static function purgeCache()
    {
        ...
        $driver->setNamespace(PluginsTableMap::TABLE_NAME);
            ...
            if ($result instanceof \ArrayAccess) {
                foreach ($result as $element) {
                    if ($element instanceof \Plugins) {
                        PluginsTableMap::addInstanceToPool($element);
                    }
                }
            } else if ($result instanceof \Plugins) {
                PluginsTableMap::addInstanceToPool($result);
            }
        }
    ...
    {
        if ($this->isCacheEnable() && $cache = \PluginsQuery::cacheFetch($this->getCacheKey())) {
        ...
        if ($this->isCacheEnable()) {
            \PluginsQuery::cacheStore($this->getCacheKey(), $data, $this->getLifeTime());
        }
